...
/// AppointmentRepository — Handles all Firestore operations for field visits
...
class AppointmentRepository extends GetxService {
  ...
  Future<List<UserModel>> fetchExperts() async {
    try {
      debugPrint('[AppointmentRepository] Fetching experts...');
      ...
      debugPrint(
        '[AppointmentRepository] Primary query returned ${snapshot.docs.length} docs',
      );
      ...
      if (experts.isEmpty) {
        debugPrint(
          '[AppointmentRepository] Primary query empty, trying fallback...',
        );
        return _fetchExpertsFallback();
      }

      debugPrint('[AppointmentRepository] Found ${experts.length} experts');
      return experts;
    } on FirebaseException catch (e) {
      // Handle missing Firestore composite index gracefully
      if (e.code == 'failed-precondition') {
        debugPrint(
          '⚠️ [AppointmentRepository] Missing Firestore index for '
      ...
      debugPrint(
        '[AppointmentRepository] FirebaseException fetching experts: '
        '${e.code} - ${e.message}',
      );
      // Even on other Firebase errors, try the simpler fallback
      return _fetchExpertsFallback();
    } catch (e) {
      debugPrint('[AppointmentRepository] Error fetching experts: $e');
  ...
  Future<List<UserModel>> _fetchExpertsFallback() async {
    try {
      debugPrint('[AppointmentRepository] Running fallback expert fetch...');
      final snapshot = await _firestore
          .collection('users')
          .where('userType', isEqualTo: 'expert')
          .get();
      ...
      debugPrint(
        '[AppointmentRepository] Fallback query returned ${snapshot.docs.length} docs',
      );
      ...
      debugPrint(
        '[AppointmentRepository] Fallback found ${experts.length} valid experts '
        'out of ${allExperts.length} total',
      );
      return experts;
    } catch (e) {
      debugPrint('[AppointmentRepository] Fallback error fetching experts: $e');
  ...
  List<UserModel> _parseUserDocs(List<QueryDocumentSnapshot> docs) {
    ...
    for (final doc in docs) {
      try {
        ...
        debugPrint(
          '⚠️ [AppointmentRepository] Failed to parse user doc '
          '${doc.id}: $e',
        );
      }
    }
  ...
  Future<UserModel?> fetchExpertById(String expertId) async {
    try {
      ...
      debugPrint('[AppointmentRepository] Error fetching expert: $e');
      rethrow;
    }
  }
  ...
  Future<String> createVisitRequest(FieldVisitModel visit) async {
    try {
      debugPrint('[AppointmentRepository] Creating visit request...');
      final docRef = await _visitsCollection.add(visit.toDocument());
      debugPrint('[AppointmentRepository] Visit created: ${docRef.id}');
      return docRef.id;
    } catch (e) {
      debugPrint('[AppointmentRepository] Error creating visit: $e');
      rethrow;
    }
  }
  ...
  Future<List<FieldVisitModel>> fetchFarmerVisits(String farmerId) async {
    try {
      debugPrint(
        '[AppointmentRepository] Fetching visits for farmer: $farmerId',
      );
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          debugPrint(
            '⚠️ [AppointmentRepository] Failed to parse visit doc '
            '${doc.id}: $e',
          );
        }
      }
      ...
      debugPrint(
        '[AppointmentRepository] Found ${visits.length} farmer visits',
      );
      return visits;
    } on FirebaseException catch (e) {
      // Handle missing Firestore composite index gracefully
      ...
      debugPrint(
        '[AppointmentRepository] FirebaseException fetching farmer visits: '
        '${e.code} - ${e.message}',
      );
      rethrow;
    } catch (e) {
      debugPrint('[AppointmentRepository] Error fetching farmer visits: $e');
      rethrow;
    }
  }
  ...
  Future<List<FieldVisitModel>> fetchExpertVisits(
    ...
    try {
      debugPrint(
        '[AppointmentRepository] Fetching visits for expert: $expertId, '
      ...
      debugPrint(
        '[AppointmentRepository] Found ${visits.length} expert visits',
      );
      return visits;
    } on FirebaseException catch (e) {
      // Handle missing Firestore composite index gracefully
      ...
      debugPrint(
        '[AppointmentRepository] FirebaseException fetching expert visits: '
        '${e.code} - ${e.message}',
      );
      rethrow;
    } catch (e) {
      debugPrint('[AppointmentRepository] Error fetching expert visits: $e');
      rethrow;
    }
  }
  ...
  Future<FieldVisitModel?> fetchVisitById(String visitId) async {
    try {
      ...
      debugPrint('[AppointmentRepository] Error fetching visit: $e');
      rethrow;
    }
  }
  ...
  Future<void> updateVisitStatus(
    ...
    try {
      debugPrint(
        '[AppointmentRepository] Updating visit $visitId → $newStatus',
      );
      ...
      debugPrint('[AppointmentRepository] Visit status updated successfully');
    } catch (e) {
      debugPrint('[AppointmentRepository] Error updating visit status: $e');
      rethrow;
    }
  }
  ...
  Future<void> cancelVisit(String visitId) async {
    try {
      debugPrint('[AppointmentRepository] Cancelling visit: $visitId');
      ...
      debugPrint('[AppointmentRepository] Visit cancelled successfully');
    } catch (e) {
      debugPrint('[AppointmentRepository] Error cancelling visit: $e');
      rethrow;
    }
  }
}
